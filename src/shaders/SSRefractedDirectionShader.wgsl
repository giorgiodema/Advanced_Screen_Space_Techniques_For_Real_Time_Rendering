/*
Given the geometry of a volume object, returns the refracted direction, that can be used to
index a distant environment map (see BlendVolumeEnvShader.wgsl). The output vector (vec4f out) 
is structured as follows:
    if(out.w > 0)
        --> out.xyz: refracted direction
        --> out.w: distance travelled
    else:
        --> the fragment is out of the
            volume object
*/
...
fn fs(@builtin(position) fragCoord:vec4f) -> @location(0) vec4f {
    ...
    //if(dot(pivot1,unitPositionFrom1)<0.0){
    //    return vec4f(0.0);
    //}
    ...
    let posView2 = textureLoad(positionTextureLayer2,vec2<i32>(fragCoord2.xy),0).xyz;
    // handle the case when the intersection point is
    // not on the backface geometry
    //if(abs(posView2.z/maxDistance)>=1.0){
    //    return vec4f(1.0,0.0,1.0,1.0);
    //}
    let pivot2 = normalize(refract(pivot1,-normalView2,ior));
    var out = vec4f(pivot2.xyz,res1.distanceTravelled);
    let thetai = acos(dot(pivot1,normalView2));
    let thetaCrit = asin(1.0/1.5);
    // handle total internal reflection
    if(thetai >= thetaCrit){
        return vec4f(normalView2,res1.distanceTravelled);
    }
    return out;
}
