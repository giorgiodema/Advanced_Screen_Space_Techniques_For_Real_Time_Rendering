...
fn fs(@builtin(position) fragCoord:vec4f) -> @location(0) vec4f {
    ...
    // this fills gaps when the refracted color is not 
    // visible due to the limitations of Screen Space 
    // refraction
    if(volumeRefractionUV.w != 0.0 ){
        transmitted = attenuation * attenuationColor * opaque;
    }
    ...
    //let visRefr = pow(Vis_SmithTransmission(a2, NoV, NoT, HToT, HToV),0.3);
    //let diffuseTransmitted = visRefr * mix(color,transmitted,transmission);
